...
functionality.
"""
BIDSDataGrabber = pe.Node(util.Function(function=utils.get_files_spm, 
                                    input_names=["subject_id", "session", "task", "raw_data_dir"],
                                    output_names=["bolds", "struct", "events", "TR"]), 
...
tsv2subjinfo = pe.Node(util.Function(function=utils.tsv2subjectinfo, input_names=['in_file'],
                         output_names=['subject_info']), name="tsv2subjinfo")
...
#normalize.inputs.template = os.path.abspath('/Applications/spm12/canonical/avg152T1.nii')
...
level1design.inputs.use_mcr = False
...
l1pipeline.base_dir = os.path.abspath('hemi_spm_full/workingdir')

l1pipeline.connect([
    (infosource, BIDSDataGrabber, [('subject_id', 'subject_id')]),
    (BIDSDataGrabber, realign, [('bolds', 'in_files')]),
    (realign, coregister, [('mean_image', 'source'), ('realigned_files',
                                                      'apply_to_files')]),
    (BIDSDataGrabber, coregister, [('struct', 'target')]),
    (BIDSDataGrabber, normalize, [('struct', 'source'), ('struct', 'template')]),
    (coregister, normalize, [('coregistered_files', 'apply_to_files')]),
    (normalize, smooth, [('normalized_files', 'in_files')]),
    (BIDSDataGrabber, modelspec, [('events', 'subject_info'),
                                  ('TR', 'time_repetition')]),
    (realign, modelspec, [('realignment_parameters',
                           'realignment_parameters')]),
    (smooth, modelspec, [('smoothed_files', 'functional_runs')]),
    (normalize, skullstrip, [(('normalized_files', utils.pickfirst), 'in_file')]),
    ...
    (BIDSDataGrabber, level1design, [('TR', 'interscan_interval')]),
...
datasink.inputs.base_directory = os.path.abspath('hemi_spm_full/l1output')


def getstripdir(subject_id):
    import os
    return os.path.join(
        os.path.abspath('hemi_spm_full/workingdir'),
...
#     'hemi_spm_full/l1output/*/con*/*/_fwhm_%d/con_%04d.*i*')
...
# l2pipeline.base_dir = os.path.abspath('hemi_spm_full/l2output')
# l2pipeline.connect([
